...
export function useSounds() {
  ...
  const playTone = useCallback((freq: number, duration: number, type: OscillatorType = 'sine', volume = 0.15, delay = 0) => {
    try {
      const ctx = getCtx();
  ...
  const playWin = useCallback(() => {
    ...
    // Sparkle overlay
    playTone(1568, 0.1, 'sine', 0.06, 0.45);
    playTone(2093, 0.1, 'sine', 0.05, 0.55);
  }, [playTone]);

  // 😢 Lose sound - gentle descending
  const playLose = useCallback(() => {
    playTone(440, 0.2, 'sine', 0.1, 0);
    playTone(370, 0.2, 'sine', 0.1, 0.2);
    playTone(311, 0.3, 'sine', 0.1, 0.4);
  }, [playTone]);

  // ❌ Wrong guess - soft bonk
  const playWrong = useCallback(() => {
    playTone(200, 0.12, 'triangle', 0.15, 0);
    playTone(150, 0.15, 'triangle', 0.1, 0.05);
  }, [playTone]);
  ...
  const playFlip = useCallback(() => {
    playTone(1200, 0.04, 'sine', 0.08, 0);
    playTone(800, 0.03, 'sine', 0.06, 0.02);
  }, [playTone]);

  // 💫 Match found - sparkle sound
  const playMatch = useCallback(() => {
    playTone(1047, 0.08, 'sine', 0.1, 0);
    playTone(1319, 0.08, 'sine', 0.1, 0.06);
    playTone(1568, 0.12, 'sine', 0.12, 0.12);
  }, [playTone]);

  // 🤝 Draw - neutral chime
  const playDraw = useCallback(() => {
    playTone(440, 0.15, 'sine', 0.08, 0);
    playTone(440, 0.15, 'sine', 0.08, 0.2);
  }, [playTone]);

  // 💬 Chat message - soft ping
  const playMessage = useCallback(() => {
    playTone(1200, 0.06, 'sine', 0.06, 0);
